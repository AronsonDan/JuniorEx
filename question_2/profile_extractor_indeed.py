...
class ProfileExtractorIndeed(ProfileExtractor):
    ...
    def set_skills(self):
        ...
        if has_skill_text():
            ...
            # skills_string = ''.join(has_additional_info().find('br').next_siblings)
            # for skill_item in skills_string.split(','):
            #     self.skills.append(skill_item)
            print(has_additional_info())


        # print(has_skill_text())
        # print(has_additional_info())
        # skills_string = ''.join(skills_section.find('br').next_siblings)
        # for skill_item in skills_string.split(','):
        #     self.skills.append(skill_item)
    ...
    def set_education(self):
        ...
        def find_years(bs4_element):
            years_element = bs4_element.find('p', {'class': "edu_title"})
            if years_element:
                return years_element.text
            else:
                return None

        for element in self.html_page.find_all('div', {'class': "education-section "}):
